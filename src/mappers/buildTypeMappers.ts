...
export function buildTypeMappers(pathToMap: string | string[], mapperName = 'GraphQLTypes'): Record<string, string> {
  if (!pathToMap || pathToMap.length === 0) {
    throw new Error(
      'Please enter one or more paths to files containing your TypeScript mappers declaration. See documentation for details.'
    );
  }
